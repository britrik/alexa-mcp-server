...
export async function getAlexaSessionCredentials(env: Env): Promise<AlexaSessionCredentials | null> {
	...
	if ((typeof env.UBIDMAIN === "string" && typeof env.ATMAIN === "string") || (typeof env.UBID_MAIN === "string" && typeof env.AT_MAIN === "string")) {
		return {
			ubidMain: (env.UBIDMAIN ?? env.UBID_MAIN) as string,
			atMain: (env.ATMAIN ?? env.AT_MAIN) as string,
